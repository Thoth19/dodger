...
# =============================
#   Dodger Environment Wrapper
# =============================
class DodgerEnv(object):
    """
    Small wrapper for environment.
    """
    ...
    def get_initial_state(self):
        """
        Resets the dodger game, clears the state buffer.
        """
        # Clear the state buffer
        self.state_buffer = deque()

        self.game.reset()
        x_t = self.get_state_vector()
        # Stack of game frames. Note that in a arcade game we care 
        # about multiple frames because the buttons need to be held down before
        # the action actually means anything. Here we want it in case
        # we change how lookahead works.
        # atm its a stack of ONE element
    ...
    def get_state_vector(self):
        """
        Gets the data out of the game
        """
        g = self.game.dangers
        g.append(self.game.player.pos)
        return g

    def step(self, action_index):
        """
        Excecutes an action in the environment.
        ...
        # Game steps, and we get data out of that
        self.game.step()
        x_t1 = self.get_state_vector()

        previous_frames = np.array(self.state_buffer)
        s_t1 = np.empty((self.action_repeat))
...
def actor_learner_thread(thread_id, env, session, graph_ops, num_actions,
    ...
    # Wrap env with DodgerEnv helper class
    player = LearningPlayer(5)
    game = Game(10, player)
    env = DodgerEnv(game, action_repeat)
